...
def state0(N, i): #preparing the |0> state of the i-th element of a chain with length N
	if i == 0:
...
def state1(N, i): #preparing the |1> state of the i-th element of a chain with length N
	if i == 0:
		sigma0 = np.array([[0.,0.],[0.,1.]])
	else:
		sigma0 = np.eye(2)
	if i != 0:
		for n in range(0,i-1):
			sigma0 = np.kron(sigma0, np.eye(2))
		sigma0 = np.kron(sigma0, np.array([[0.,0.],[0.,1.]]))
		for n in range(i,N):
			sigma0 = np.kron(sigma0, np.eye(2))
		return sigma0
	else:
